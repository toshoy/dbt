with source as (
      select 
      YEAR,
      PERIOD,
      ACCOUNT,
      ENTITY,
      CUSTOMER,
      PRODUCT,
      SUM(AMOUNT) AS AMOUNT      
      from {{ ref('_stg_test_CR_epm_IN_dev_aida') }} --{{ ref('_stg__EPM_ALL_SCENARIOS') }} temporary fix for the performance issue
      --where currency = 'USD' AND SCENARIO = 'OEP_Actual' AND "VERSION" = 'PostAlloc' and len(PERIOD) = 3
      GROUP BY ALL),
mapping as (
      SELECT *
      FROM {{ ref('TEST_account_mapping') }}),
source_mapping as (
      SELECT 
      scr.YEAR,
      scr.PERIOD,
      scr.CUSTOMER,
      scr.PRODUCT,
      scr.AMOUNT,
      scr.ENTITY,
      COALESCE(UPPER(map."Group"), 'NO MAPPING') as "Group"
      FROM source as scr
      LEFT JOIN mapping as map ON scr.ACCOUNT = map."Account")
,pivot as (
      select 
      DATE_FROM_PARTS(RIGHT(YEAR,2)::INT+2000, TO_CHAR(TO_DATE(PERIOD, 'MON'), 'MM')::INT , 1)  AS "DATE_AS_OF",
      CUSTOMER,
      PRODUCT,
      ENTITY,
      COALESCE(ROUND("'DISCOUNTS AND ALLOWANCES'",2),0) AS "DISCOUNTS AND ALLOWANCES",
      COALESCE(ROUND("'FILL RATE FINES'",2),0) AS "FILL RATE FINES",
      COALESCE(ROUND("'FREIGHT'",2),0) AS "FREIGHT",
      COALESCE(ROUND("'GROSS SALES'",2),0) AS "GROSS SALES",
      COALESCE(ROUND("'GTN OTHER'",2),0) AS "GTN OTHER",
      COALESCE(ROUND("'NO MAPPING'",2),0) AS "NO MAPPING",
      COALESCE(ROUND("'OCOS'",2),0) AS "OCOS",
      COALESCE(ROUND("'REBATES'",2),0) AS "REBATES",
      COALESCE(ROUND("'RETURNS'",2),0) AS "RETURNS",
      COALESCE(ROUND("'ROYALTIES'",2),0) AS "ROYALTIES",
      COALESCE(ROUND("'RSA'",2),0) AS "RSA",
      COALESCE(ROUND("'SG&A'",2),0) AS "SG&A",
      COALESCE(ROUND("'STD. MTL. COST'",2),0) AS "STD. MTL. COST"
      from source_mapping
      pivot(sum(AMOUNT) for "Group" in (any order by  "Group"))
      GROUP BY ALL),
TOTALS AS (
      SELECT
      DATE_AS_OF as F_MONTH_DATE,
      CONCAT(DM."GPP_DIVISION_CODE",CM.COE_Market_Channel) AS BAR_KEY,
      UPPER(TT.CUSTOMER) AS "CUSTOMER",
      CM.COE_Market_Channel AS "MARKET_CHANNEL",
      TT.PRODUCT,
      TT.ENTITY,
      ENT.BAR_REGION,
      ENT.BAR_ENT_KEY,
      DM."GPP_DIVISION_CODE",
      DM."GPP_DIVISION_DESCR",
      "GROSS SALES",
      "RETURNS",
      "RSA",
      "ROYALTIES",
      "REBATES",
      "DISCOUNTS AND ALLOWANCES",
      "FILL RATE FINES",
      "GTN OTHER",
      "FILL RATE FINES"+"DISCOUNTS AND ALLOWANCES"+"GTN OTHER" AS "TOTAL GTN",
      "GROSS SALES" - "RSA" - "RETURNS" + "ROYALTIES" - "TOTAL GTN" AS "TOTAL NSV",
      "STD. MTL. COST",
      "FREIGHT",
      "FREIGHT" + "STD. MTL. COST" AS "TOTAL COST",
      "TOTAL NSV" - "TOTAL COST" AS "SGM",
      "OCOS",
      "SG&A",
      "NO MAPPING"
      FROM pivot AS TT
      LEFT JOIN {{ ref('TEST_BAR_DIV_MAPPING') }} AS DM ON TT.PRODUCT = DM.PRODUCT
      LEFT JOIN {{ ref('TEST_BAR_CUS_MAPPING') }} AS CM ON UPPER(TT.CUSTOMER) = CM.CUSTOMER
      LEFT JOIN {{ ref('_stg_ent_map_finance') }} AS ent on UPPER(TT.ENTITY) = UPPER(ent.BAR_ENT)
),
REGION_MAPPING AS (
      SELECT TT.*,
      MAP.REGION AS "REGION_FINANCE_MAP",
      CASE
      WHEN DCT.BAR_ENTITY_REGION IN ('North America') THEN 'US'    
      ELSE DCT.BAR_ENTITY_REGION END AS "REGION_DCT_MAP",
      COALESCE(MAP.REGION,"REGION_DCT_MAP",'NO ENTITY MAPPING') AS "REGION",
      CASE 
      WHEN MAP.REGION IS NOT NULL THEN 'DEV_AIDA.OPG_PL.ENT_MAP'
      WHEN MAP.REGION IS NULL AND DCT.BAR_ENTITY_REGION IS NOT NULL THEN 'PROD_RAW.DCT.DCT_BAR_ENTITY_ATTR'
      WHEN MAP.REGION IS NULL AND DCT.BAR_ENTITY_REGION IS NULL THEN 'NO_MAPPING'
      END AS "REGION_MAPPING_SOURCE"
      FROM TOTALS AS TT
      LEFT JOIN {{ ref('TEST_BAR_ENT_MAP') }} AS MAP ON TT.BAR_ENT_KEY = MAP.BAR_ENT_KEY
      LEFT JOIN {{ ref('_stg_DCT_mapping') }} AS DCT ON TT.ENTITY = DCT.BAR_ENTITY
      WHERE ("DISCOUNTS AND ALLOWANCES"+"FILL RATE FINES"+"FREIGHT"+"GROSS SALES"+"GTN OTHER"+ "OCOS"+"REBATES"+"RETURNS"+"ROYALTIES"+"RSA"+"SG&A"+"STD. MTL. COST") <> 0 ), -- no mapping removed from the filter
FINAL AS (
      SELECT *,
      CASE
      WHEN REGION IN ('US','CANADA') THEN BAR_KEY 
      WHEN "GPP_DIVISION_CODE" IS NULL THEN NULL
      ELSE UPPER(CONCAT(COALESCE(TRIM("GPP_DIVISION_CODE"),''), COALESCE(TRIM("REGION"),''))) 
      END  AS "BAR_KEY_ADJ"
      FROM REGION_MAPPING
)
SELECT *
FROM FINAL